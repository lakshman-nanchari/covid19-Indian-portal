...
const convertSateDbObjectToResponseObject = (dbObject) => {
  return {
...
app.get("/states/", authenticateToken, async (request, response) => {
  ...
  response.send(
    statesArray.map((eachState) =>
      convertSateDbObjectToResponseObject(eachState)
    )
  );
});
...
app.get("/states/:stateId/", authenticateToken, async (request, response) => {
  ...
  const getSateQuery = `
    ...
    WHERE
      state_id = ${stateId};`;
  const state = await database.get(getSateQuery);
  response.send(convertSateDbObjectToResponseObject(state));
});
